...
class AppearanceController
{
    ...
    public function save(Request $request)
    {
        $this->request->validate([
            'favicon' => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|dimensions:min_width=64,min_height=64',
            'logo' => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|dimensions:min_width=256,min_height=256',
        ]);

        if ($this->request->hasFile('favicon')) {
            $favicon = Image::make($this->request->file('favicon'));
            $favicon->fit(64, 64);
            $favicon->save(public_path('/favicon.png'));
        }

        if ($this->request->hasFile('logo')) {
            $favicon = Image::make($this->request->file('logo'));
            $favicon->fit(256, 256);
            $favicon->save(public_path('assets/images/logo.png'));
        }
