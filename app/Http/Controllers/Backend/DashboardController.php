...
class DashboardController
{
    public function index(Request $request)
    {
        ...
        if(intval($maxFileSize) < 20) $maxFileSize = "<span class=\"text-danger\">{$maxFileSize}</span>";
        else $maxFileSize = " <span class=\"text-success\">{$maxFileSize}</span>";

        $dashboard->information = "";
        $dashboard->information .="<tr><td>AI Engine Version</td><td><strong>" . env('APP_VERSION') . "</strong></td></tr>\n";
        ...
        $stats = Subscription::select(DB::raw('sum(amount) AS revenue'))->first();
