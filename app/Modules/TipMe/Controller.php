...
class Controller
{
    ...
    {
        $this->endpoint = config('payment.gateways.tipme.endpoint');
        $this->_protocol = $protocol;
        $this->request = $request;
        $this->_APIKey = config('payment.gateways.tipme.public_key');
    ...
    protected function getAuthToken()
    {
        if(! $this->request->session()->get('authToken')) {
            ...
            $response = Http::withHeaders([
                'authorizekey' => config('payment.gateways.tipme.api_key')
            ])
                ->asForm()
                ->post($this->endpoint . '/Business_api/authenticate', [
                    "merchant_id" => config('payment.gateways.tipme.merchant_id'),
                ]);
