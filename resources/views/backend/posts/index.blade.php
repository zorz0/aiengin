...
@section('content')
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">{{ __('admin.posts', ['count' => $posts->total()]) }}</li>
        <li class="breadcrumb-item active">{{ __('admin.edit_news') }}</li>
    </ol>
    <div class="row">
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 border-0">
                    <button class="btn btn-link p-0 m-0" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                        <h6 class="m-0 font-weight-bold text-primary">{{ __('admin.advanced_search') }}</h6>
                    </button>
                    <a href="{{ route('backend.posts.add') }}" class="btn btn-primary btn-sm float-right">{{ __('admin.add_new_article') }}</a>
                </div>
                                        ...
                                        <select class="form-control multi-selector-without-sortable" data-ajax--url="/api/search/user" name="userIds[]" multiple="">
                                            @if(request()->input('userIds') && is_array(request()->input('userIds')))
                                                @foreach (\App\Models\User::whereIn('id', request()->input('userIds'))->get() as $index => $user)
                                                    <option value="{{ $user->id }}" selected="selected" data-artwork="{{ $user->artwork_url ?  $user->artwork_url : url('common/default/user.svg') }}" data-title="{{ $user->name }}">{{ $user->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
            ...
            <form id="mass-action-form" method="post" action="{{ route('backend.posts.mass.action') }}">
                ...
                <div class="row">
                    <div class="col-6">{{ $posts->appends(request()->input())->links('pagination::bootstrap-4', ['class' => 'pagination-nav']) }}</div>
                    <div class="col-6">
